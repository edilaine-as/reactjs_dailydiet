...
export function DietOnDietPercentualCard({ className = "", icon = false }) {
	...
	const percent = metricsUser?.totalDiets
		? Math.round(
				(metricsUser?.totalDietsOnDiet * 100) / metricsUser?.totalDiets,
			)
		: 0;
	...
	return (
		<Dialog>
			<Card
				className={`bg-custom-green-200 dark:bg-custom-green-300 border-none shadow-none ${className}`}
			>
				...
				<CardHeader className="pb-1">
					<CardTitle className="text-2xl text-center text-custom-gray-600 dark:text-white">
						{percent}%
					</CardTitle>
				</CardHeader>
			...
			<DialogContent
				className="flex content-start h-full w-full p-0"
